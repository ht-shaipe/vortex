...
pub fn list(conn: &rusqlite::Connection) -> Result<Vec<ApiKey>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, key, machine_id, allowed_models, allowed_combos, \
...
pub fn get_by_key(conn: &rusqlite::Connection, key: &str) -> Result<Option<ApiKey>> {
    let mut stmt = conn.prepare(
        "SELECT id, name, key, machine_id, allowed_models, allowed_combos, \
         allowed_connections, allowed_endpoints, no_log, auto_resolve, \
         is_active, is_banned, rate_limits, usage_limits, created_at \
         FROM api_keys WHERE key = ?1 AND is_active = 1 AND is_banned = 0"
    )?;
...
pub fn create(conn: &rusqlite::Connection, req: &CreateApiKeyRequest) -> Result<ApiKey> {
    ...
    let allowed_combos = serde_json::to_string(&req.allowed_combos.clone().unwrap_or(serde_json::json!([])))?;
    ...
    conn.execute(
        "INSERT INTO api_keys (id, name, key, allowed_models, allowed_combos, \
         allowed_connections, allowed_endpoints, no_log, auto_resolve, \
         is_active, is_banned, rate_limits, usage_limits, created_at) \
         VALUES (?1, ?2, ?3, ?4, ?5, '[]', '[]', 0, 1, 1, 0, ?6, ?7, ?8)",
        params![id, req.name, key, allowed_models, allowed_combos, rate_limits, usage_limits, now],
    )?;
    ...
    Ok(ApiKey {
        ...
        allowed_combos: req.allowed_combos.clone().unwrap_or(serde_json::json!([])),
...
fn row_to_key(row: &Row) -> rusqlite::Result<ApiKey> {
    ...
    let allowed_combos_str: String = row.get(5)?;
    let allowed_conn_str: String = row.get(6)?;
    let allowed_endpoints_str: String = row.get(7)?;
    let rate_limits_str: String = row.get(12)?;
    let usage_limits_str: String = row.get(13)?;
    ...
    Ok(ApiKey {
        ...
        allowed_combos: serde_json::from_str(&allowed_combos_str).unwrap_or(serde_json::json!([])),
        ...
        no_log: row.get::<_, i32>(8)? != 0,
        auto_resolve: row.get::<_, i32>(9)? != 0,
        is_active: row.get::<_, i32>(10)? != 0,
        is_banned: row.get::<_, i32>(11)? != 0,
