...
pub async fn list_profiles(state: web::Data<std::sync::Arc<AppState>>) -> HttpResponse {
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        ...
        crate::db::routing_profiles::list(&conn)
...
pub async fn create_profile(
    ...
    body: web::Json<crate::db::routing_profiles::CreateRoutingProfile>,
) -> HttpResponse {
    let pool = state.db_pool.clone();
    let req = body.into_inner();
    let result = tokio::task::spawn_blocking(move || -> Result<crate::db::routing_profiles::RoutingProfile, String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        crate::db::routing_profiles::create(&conn, &req).map_err(|e| e.to_string())
    })
    .await;
    match result {
...
pub async fn delete_profile(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<(), String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        crate::db::routing_profiles::delete(&conn, &id).map_err(|e| e.to_string())
    })
    .await;
    match result {
