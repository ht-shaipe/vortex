...
pub async fn list_rate_limits(state: web::Data<std::sync::Arc<AppState>>) -> HttpResponse {
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        ...
        crate::db::rate_limits::list_caps(&conn)
...
pub async fn upsert_rate_limit(
    ...
    body: web::Json<crate::db::rate_limits::UpsertRateLimitCap>,
) -> HttpResponse {
    let pool = state.db_pool.clone();
    let req = body.into_inner();
    let rate_limiter = state.rate_limiter.clone();
    let result = tokio::task::spawn_blocking(move || -> Result<crate::db::rate_limits::RateLimitCap, String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        let cap = crate::db::rate_limits::upsert_cap(&conn, &req).map_err(|e| e.to_string())?;
...
pub async fn delete_rate_limit(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<(), String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        crate::db::rate_limits::delete_cap(&conn, &id).map_err(|e| e.to_string())
    })
    .await;
    match result {
...
pub async fn list_usage(state: web::Data<std::sync::Arc<AppState>>) -> HttpResponse {
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        crate::db::rate_limits::list_usage_snapshots(&conn)
...
pub async fn cleanup_usage(state: web::Data<std::sync::Arc<AppState>>) -> HttpResponse {
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        let conn = crate::db::core::get_conn(&pool).map_err(|e| e.to_string())?;
        let deleted = crate::db::rate_limits::cleanup_old_usage(&conn).map_err(|e| e.to_string())?;
