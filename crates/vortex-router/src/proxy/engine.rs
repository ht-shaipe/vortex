...
use vortex_store::db::models::{ProxyRequest, UsageEntry};
use vortex_store::db::{core as db_core, model_aliases as db_aliases, providers as db_providers, rate_limits as db_rate_limits, routing_profiles as db_routing_profiles, settings as db_settings, usage as db_usage};
use vortex_store::error::{AppError, Result};
...
impl ProxyEngine {
    ...
    pub async fn handle_request(
        ...
        if state.config().require_api_key {
            if let Some(ref api_key) = request.api_key {
                ...
                if !vortex_store::db::key_permissions::check_access(&conn, &key_obj.id, &request.model)? {
        ...
        // Prompt 压缩：从 settings 读取压缩级别（fail-open + never-worse 保护）
        {
            let comp_config = {
                ...
                let level_str = tokio::task::spawn_blocking(move || -> String {
                    let Ok(conn) = vortex_store::db::core::get_conn(&pool) else { return "minimal".to_string() };
                    let val = vortex_store::db::settings::get(&conn, "settings", "general").unwrap_or(None);
                    val.as_ref()
            ...
            if saved > 0 {
                ...
                tokio::task::spawn_blocking(move || {
                    let Ok(conn) = vortex_store::db::core::get_conn(&pool) else { return };
                    let content = serde_json::to_string(&original_messages).unwrap_or_default();
                    let hash = vortex_store::db::compressed_content::content_hash(&content);
                    let _ = vortex_store::db::compressed_content::store(
    ...
    async fn handle_emergency_failover(
        ...
        failed_conn: &vortex_store::db::models::ProviderConnection,
        failed_model: &str,
    ...
    async fn handle_single_with_retry(
        ...
        connection: &vortex_store::db::models::ProviderConnection,
        model: &str,
        start: Instant,
...
use vortex_store::db::api_keys as db_api_keys;
