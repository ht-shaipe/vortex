...
use vortex_store::db::models::ModelAliasTarget;
...
pub fn run_auto_grouping(
    ...
    enc_key: &[u8],
) -> vortex_store::error::Result<Vec<(String, Vec<ModelAliasTarget>)>> {
    // 收集所有活跃连接的模型
    let all_conns = vortex_store::db::providers::list(conn, enc_key)?;
    ...
    // 删除旧 auto 别名 + 创建新别名
    vortex_store::db::model_aliases::delete_all_auto(conn)?;
    vortex_store::db::model_aliases::create_auto_batch(conn, &groups)?;
