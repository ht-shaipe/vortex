...
use vortex_store::db::model_catalog::{self, CatalogEntry};
use vortex_store::error::Result;
...
pub async fn refresh_from_connections(
    pool: &vortex_store::db::core::DbPool,
    enc_key: &[u8],
    ...
    let connections = tokio::task::spawn_blocking(move || -> std::result::Result<Vec<vortex_store::db::models::ProviderConnection>, String> {
        let conn = vortex_store::db::core::get_conn(&pool_clone).map_err(|e| e.to_string())?;
        vortex_store::db::providers::list_all_active(&conn, &enc_key_clone).map_err(|e| e.to_string())
    })
    .await
    .map_err(|e| vortex_store::error::AppError::Internal(format!("DB task failed: {}", e)))?
    .map_err(vortex_store::error::AppError::Internal)?;
    ...
    if !all_entries.is_empty() {
        ...
        tokio::task::spawn_blocking(move || -> std::result::Result<(), String> {
            let conn = vortex_store::db::core::get_conn(&pool_clone2).map_err(|e| e.to_string())?;
            model_catalog::batch_upsert(&conn, &entries_clone).map_err(|e| e.to_string())
        })
        .await
        .map_err(|e| vortex_store::error::AppError::Internal(format!("DB write task failed: {}", e)))?
        .map_err(vortex_store::error::AppError::Internal)?;
    }
