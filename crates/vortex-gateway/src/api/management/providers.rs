...
/// - `c`：数据库中的提供商连接对象
/// - 返回值：脱敏后的 JSON 值
fn mask_connection(c: &vortex_store::db::models::ProviderConnection) -> serde_json::Value {
...
pub async fn create_provider(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<serde_json::Value, String> {
        ...
        let _ = vortex_router::providers::auto_grouping::run_auto_grouping(&conn, &encryption_key);
...
pub async fn test_provider(
    ...
    // 1. DB 读取
    let provider_result = tokio::task::spawn_blocking(move || -> Result<Option<vortex_store::db::models::ProviderConnection>, String> {
        let conn = db_core::get_conn(&pool).map_err(|e| e.to_string())?;
...
async fn test_connection(
    ...
    provider_def: Option<&vortex_router::providers::types::ProviderDef>,
    client: &awc::Client,
) -> TestResult {
    // 获取提供商定义，未知则返回错误
