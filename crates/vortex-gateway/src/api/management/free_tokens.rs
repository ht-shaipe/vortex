...
use vortex_store::db::{core as db_core, free_tokens as db_free_tokens};
use vortex_store::db::models::CreateFreeTokenSiteRequest;
use vortex_router::remote_proxy;
use crate::AppState;
...
fn hub_headers(config: &vortex_store::config::AppConfig) -> HashMap<String, String> {
...
/// - `pool`：数据库连接池
/// - `req`：创建站点的请求体
fn save_local_copy(pool: &vortex_store::db::core::DbPool, req: &CreateFreeTokenSiteRequest) {
...
pub async fn create_site(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<vortex_store::db::models::FreeTokenSite, String> {
        let conn = db_core::get_conn(&pool).map_err(|e| e.to_string())?;
