...
use vortex_router::proxy::sse::{sse_http_response, SseStream};
use crate::AppState;
        ...
        .await
    {
        Ok(ProxyOutput::Response(body)) => {
            ...
            HttpResponse::Ok().json(
                vortex_router::translator::openai_to_anthropic_response(&body, &original_model),
            )
        }
...
/// - `body`：Anthropic 格式 JSON 请求体
/// - `req`：HTTP 请求，用于提取鉴权头
/// - 返回值：解析成功的 [`ProxyRequest`] 或 [`AppError`](vortex_store::error::AppError)
fn parse_anthropic_request(
    ...
    let mut openai_body = vortex_router::translator::anthropic_request_to_openai_body(body);
