...
use vortex_router::proxy::sse::SseStream;
use crate::AppState;
...
pub async fn responses(
    ...
    match app_state
        ...
        .await
    {
        Ok(vortex_router::proxy::engine::ProxyOutput::Response(body)) => {
...
fn parse_responses_request(
    ...
    req: &HttpRequest,
) -> vortex_store::error::Result<ProxyRequest> {
    // 提取模型名（必填）
    let model = body
        .get("model")
        .and_then(|v| v.as_str())
        .ok_or_else(|| vortex_store::error::AppError::BadRequest("model is required".into()))?
        .to_string();
...
/// - 字符串：转换为单条 user 消息
/// - 数组：包含多种类型的输入项
fn convert_input_to_messages(body: &serde_json::Value) -> vortex_store::error::Result<serde_json::Value> {
    let input = body.get("input").ok_or_else(|| {
        vortex_store::error::AppError::BadRequest("input is required".into())
    })?;
    ...
    let messages = if let Some(text) = input.as_str() {
        ...
        return Err(vortex_store::error::AppError::BadRequest(
...
/// - 其他类型：暂不支持
fn convert_input_item(item: &serde_json::Value) -> vortex_store::error::Result<Option<serde_json::Value>> {
    ...
    match item_type {
        "message" => {
            ...
            let content = item.get("content").ok_or_else(|| {
                vortex_store::error::AppError::BadRequest("message content is required".into())
            })?;
        ...
        "function_call_output" => {
            ...
            let call_id = item.get("call_id").and_then(|c| c.as_str()).ok_or_else(|| {
                vortex_store::error::AppError::BadRequest("function_call_output requires call_id".into())
            })?;
...
) -> vortex_store::error::Result<serde_json::Value> {
...
fn convert_to_responses_format(
    ...
    let choices = chat_response
        ...
        .ok_or_else(|| {
            vortex_store::error::AppError::Provider("Invalid chat response format".into())
        })?;
    ...
    for choice in choices {
        let message = choice.get("message").ok_or_else(|| {
            vortex_store::error::AppError::Provider("Missing message in choice".into())
        })?;
        ...
        if let Some(tool_calls) = message.get("tool_calls").and_then(|t| t.as_array()) {
            ...
            for call in tool_calls {
                ...
                let function = call.get("function").ok_or_else(|| {
                    vortex_store::error::AppError::Provider("Missing function in tool call".into())
                })?;
...
fn convert_to_responses_stream(
    ...
    vortex_router::proxy::sse::sse_http_response(responses_stream)
}
