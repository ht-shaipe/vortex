...
use vortex_store::db::models::UsageEntry;
use crate::AppState;
use serde_json::json;
use std::sync::Arc;
    ...
    match req.send_json(&req_body).await {
        Ok(mut response) => {
            ...
            let entry = UsageEntry {
                ...
                cost: 0.0, usage_estimated: false, saved_tokens: 0,
                agent,
                timestamp: chrono::Utc::now().to_rfc3339(),
            };
            // best-effort 写入用量记录，失败不影响响应
            let _ = db_core::get_conn(&state.db_pool).ok().and_then(|c| vortex_store::db::usage::record(&c, &entry).ok());
