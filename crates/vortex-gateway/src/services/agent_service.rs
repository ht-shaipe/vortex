//! 智能体集成业务逻辑服务。
//!
//! 提供智能体检测、配置预览/应用/恢复、备份列表等操作，
//! 供 Tauri IPC 命令和 HTTP 端点复用。
...
use crate::agent_integrations::{
    ...
    ModelInfo, RestoreResult, VortexGatewayConfig,
};
...
/// 检测所有智能体。
pub fn detect_agents() -> Result<DetectAgentsResponse, String> {
    let start = std::time::Instant::now();
    let home = detect::get_home_dir().ok_or_else(|| "无法获取用户主目录".to_string())?;
...
/// 预览配置变更。
pub fn preview_config(state: &AppState, agent: AgentKind) -> Result<PreviewConfigResponse, String> {
    let home = detect::get_home_dir().ok_or_else(|| "无法获取用户主目录".to_string())?;
    let gateway_config = get_gateway_config(state)?;
    let registry = AdapterRegistry::new();
    let adapter = registry.get(agent).ok_or_else(|| format!("不支持的智能体类型: {:?}", agent))?;
    let preview = adapter.preview(&home, &gateway_config)?;
    Ok(PreviewConfigResponse { preview, warnings: Vec::new() })
}

/// 应用配置。
pub fn apply_config(state: &AppState, request: ApplyConfigRequest) -> Result<ApplyConfigResponse, String> {
    ...
    let registry = AdapterRegistry::new();
    let adapter = registry.get(request.agent).ok_or_else(|| format!("不支持的智能体类型: {:?}", request.agent))?;
    let result = adapter.apply(&home, &gateway_config, &backup_dir)?;
    Ok(ApplyConfigResponse { result })
}

/// 恢复配置。
pub fn restore_config(request: RestoreConfigRequest) -> Result<RestoreConfigResponse, String> {
    let home = detect::get_home_dir().ok_or_else(|| "无法获取用户主目录".to_string())?;
    let backup_dir = get_backup_dir()?;
    let registry = AdapterRegistry::new();
    let adapter = registry.get(request.agent).ok_or_else(|| format!("不支持的智能体类型: {:?}", request.agent))?;
...
/// 列出备份。
pub fn list_backups() -> Result<ListBackupsResponse, String> {
    let backup_dir = get_backup_dir()?;
    let manager = BackupManager::new(backup_dir);
    let manifests = manager.list_backups()?;
    let backups = manifests.into_iter().map(|m| BackupInfo {
        id: m.id,
        agent: m.agent,
...
/// 获取可用模型列表（从数据库查询真实模型）。
fn get_available_models(state: &AppState) -> Result<Vec<ModelInfo>, String> {
    let conn = db_core::get_conn(&state.db_pool).map_err(|e| e.to_string())?;
