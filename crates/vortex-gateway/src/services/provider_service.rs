//! 提供商业务逻辑服务。
//!
//! 提供提供商列表（含脱敏）、新增连接、测试连通性等操作，
//! 供 Tauri IPC 命令和 HTTP 管理端点复用。

use std::sync::Arc;

use serde_json::json;
use vortex_store::db::{core as db_core, models::CreateProviderRequest, providers as db_providers};

use crate::AppState;

/// 列出所有提供商定义及其连接信息（脱敏后）。
///
/// 返回 JSON 对象包含 `providers`（提供商定义数组）和 `connections`（脱敏连接数组）。
pub fn list_providers(state: &AppState) -> Result<serde_json::Value, String> {
    ...
    let mut result = vec![];
    for def in registry {
        let provider_connections: Vec<_> = connections.iter().filter(|c| c.provider == def.id).collect();
        result.push(json!({
...
/// 测试指定提供商连接的可用性。
///
/// 该函数需要 actix runtime 上下文（内部使用 awc::Client 发送 HTTP 请求）。
pub async fn test_provider(state: &Arc<AppState>, id: &str) -> Result<serde_json::Value, String> {
    let conn = db_core::get_conn(&state.db_pool).map_err(|e| e.to_string())?;
